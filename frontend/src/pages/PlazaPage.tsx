  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      <section className="rounded-[32px] liquid-panel p-6">
        ...
        <p className="mt-3 text-slate-600">지금 접속한 플레이어와 대화하고 서로의 성장 상태를 구경하세요.</p>
      ...
      <section className="rounded-[32px] liquid-panel p-6">
        ...
        <div className="mt-4 h-[420px] space-y-3 overflow-y-auto rounded-[28px] liquid-panel-soft p-4">
          {messages.map((message, index) => (
            <div key={`${message.sentAt}-${index}`} className="rounded-2xl liquid-panel-soft p-4">
        ...
        <div className="mt-4 flex gap-3">
          ...
          />
          <button
            className="rounded-2xl bg-accent px-4 py-3 text-[#35516a]"
            onClick={() => {
